window.translations.zh-CN = {
    ...
    home: {
        title: "BestToolsForever | 精密实用软件",
        studio: {
            label: "独立软件工作室",
            title: "创造 <br/> <span class=\"text-secondary\">数字持久性。</span>",
            subtitle: "由 Luiz E. L. Junior 领导的独立软件工作室。我们构建的工具优先考虑实用性、精密性以及高端软件的编辑感。",
            lead_dev: "首席开发人员",
            flagship: "旗舰产品",
        ...
        vibe: {
            ...
            feature2: "令牌优化",
            feature3: "即时 AI 提示",
            llm_ready_title: "AI 就绪上下文",
            llm_ready_desc: "一键即可为 Claude、ChatGPT 或 Gemini 捆绑您的代码。"
        },
        security: {
            badge: "Windows 原生性能",
            title: "设计即隐私",
            desc: "100% 本地运行。除非您主动发送，否则您的专有代码绝不会离开您的机器。无云端处理，无数据日志。"
        },
        features: {
            title: "精选解决方案",
            codeflatten: {
        ...
        redactor: {
            title: "智能编辑",
            desc: "安全不是建议，而是功能。在与 AI 共享之前，使用我们基于规则的内容编辑器保护 API 密钥、凭据和专有逻辑。"
        },
        profiles: {
            title: "规模化自动化",
            desc: "将您最喜欢的配置保存为配置文件。使用基于目录的覆盖功能自动为特定项目应用正确的规则。"
        },
        integration: {
            title: "无缝操作系统集成",
    ...
    support: {
        ...
        faq: {
            ...
            item1: {
                q: "出现意外的应用行为或 UI 故障？",
                a: "重新安装 Microsoft Store 包通常可以解决本地缓存一致性和更新同步问题。"
            },
            item2: {
                q: "Premium 升级有哪些好处？",
                a: "Premium 版本取消了对配置文件数量的限制，允许基于多条规则进行高级内容编辑，并启用基于目录的配置文件覆盖。"
            },
            item3: {
                q: "许可证如何管理？",
                a: "所有许可证均通过 Microsoft Store 安全管理。您的购买已关联至您的帐户，并自动同步在所有兼容的 Windows 设备上。"
            }
        },
        report: {
            title: "报告问题",
            description: "我们的工程团队会调查每一份报告。为了加快您的请求，请提供：",
            item1: "请附上错误的屏幕截图。",
            item2: "提供您的 Windows 构建版本。",
            item3: "提供问题的详细描述。",
            contact_label: "联系渠道"
        }
    },
    step5: {
        ...
        hero: {
            badge: "官方向导指南",
            title: "步骤 5 — 排除与安全",
            subtitle: "了解如何创建可靠的过滤器、减少捆绑包中的噪点，并避免大规模或危险的导出，同时不牺牲项目控制权。",
            cta1: "查看实用规则",
            cta2: "了解 Safety Scan"
        },
        quick: {
            title: "快速摘要",
            item1: "跳过列表的每一行都会针对项目名称和完整路径进行检查。",
            item2: "如果文件夹与规则匹配，则停止读取该整个分支。",
            item3: "跳过的项目可以作为占位符显示在树中，或完全隐藏。",
        ...
        safety: {
            ...
            subtitle: "步骤 5 不仅用于排除噪点。它还能防止意外导出过大、过深或危险的内容。",
            h3_files: "最大文件数",
            files: "当符合条件的文本文件计数超过配置的限制时，中断操作。",
            h3_size: "最大大小 (MB)",
            size: "当累积内容大小超过定义的天花板时，避免生成巨大的捆绑包。",
            h3_depth: "最大目录深度",
            depth: "有助于防止过度深层的树、循环或意外庞大的结构。",
            h3_guard: "Path Guard",
            guard: "默认阻止磁盘根目录和关键目录，如 C:\\ 和 C:\\Windows。",
            trigger: {
                title: "当触发限制时",
                body: "导出将安全中止。系统不会生成不完整或过于昂贵的捆绑包，而是会写入一个小文件来说明中断原因。"
            }
        },
        ai: {
            badge: "可选建议",
            ...
            prompt: {
                label: "示例请求",
                body: "您正在为 CodeFlatten PRO 的步骤 5 生成一个 skip list。\n分析仓库快照，并且只输出优化过的 fnmatch 格式的跳过列表，可供直接使用。\n\n目标：\n在排除噪音且不损害系统结构理解的前提下，生成内容最小但覆盖率最高的 skip list。\n\n优化策略：\n- 优先使用能安全移除整条噪音分支的紧凑且高效的规则。\n- 优先使用基于该仓库特征的模式，而非泛泛的捕获排除。\n- 对于孤立噪音，请使用完全一致的文件名。\n- 使用通配符匹配重复出现的文件家族。\n- 只有在明确针对非核心内容时，才使用宽泛的路径/名称模式。\n- 由于规则同时匹配目标名称与完整路径，而目录的匹配会导致整个子树被丢弃，因此当整条分支明显不必要时，首选目录级别的屏蔽。\n- 当两条规则冗余时，如果较紧凑的规则能保持相同的实际排除力，则保留它。\n- 但请勿过度压缩，以免增加丢失核心代码的风险。\n\n需要移除的内容：\n- 测试 (tests), 探针 (probes), 冒烟测试 (smoke checks), 验证辅助函数, 临时产物\n- 不涉及架构的文档和交接材料\n- 二进制资产, 图标, 构建/打包输出, 编译后的对象, 缓存 (caches), 日志 (logs), 锁文件 (locks)\n- build/dist/release 文件夹及生成的产物\n- 密钥，证书，运行时证据，商店提交残留遗留物，过多数据且对架构流理解不必要的冗余本地化文件\n\n需要保留的内容：\n- 功能核心与业务逻辑\n- 模块, 运行时, 驱动 (providers), 适配器 (adapters)\n- 编排, 入口点, 执行流\n- 任何能够说明系统如何运作的代码\n- 作为产品行为或架构核心部分的打包配置和平台整合代码\n\n判定规则：\n只要不确定，就保留代码，只移除显而易见的噪音。\n\n特殊规则：\n如果文件树标记了诸如 “[Skipped by Step 5]”、“[Skipped]”、“[Binary/Unsupported]” 等标签，请在安全的情况下，在最终列表中为其生成相应的规则。\n\n输出策略：\n- 融合两种格式：优先输出紧凑的分支级别屏蔽规则，然后跟上最精简的常见重复工件屏蔽规则。\n- 优先采用结合了仓库特定规则与普通精简清理规则的交集。\n- 彻底去重。\n- 仅输出最终的 fnmatch 模式规则，每行一条，不附带任何解释。",
                note: "将答案作为起点。理想的结果应该是可审查的、紧凑的，并与特定的转储 (dump) 目标保持一致 —— 绝不能替代人工审查或 Content Redactor。"
            }
        },
        ack: {
            title: "致谢与第三方",
        ...
        check: {
            title: "生成前的快速检查清单",
            item1: "使用精确名称进行精确排除。",
            item2: "为扩展名、测试、锁文件和循环产物使用 wildcards。",
            item3: "在大多数情况下保持 Safety Scan 开启。",
            item4: "保持 Path Guard 开启以避免根目录和关键文件夹。",
            item5: "当您需要结构上下文而不嵌入内容时，启用二进制文件引用。",
            item6: "在与第三方或 AI 共享之前，请检查最终结果。"
        },
        support: {
            label: "需要帮助？",
            title: "技术支持",
