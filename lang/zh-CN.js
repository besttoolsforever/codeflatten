window.translations['zh-CN'] = {
    ...
    step5: {
        ...
        ai: {
            ...
            prompt: {
                label: "示例请求",
                body: "使用用户的语言分析存储库快照，并仅生成采用 fnmatch 模式优化的即用型跳过列表 (skip list)，在不破坏架构的情况下排除噪音：使用精确名称删除特定文件夹或文件，对重复使用的文件系列使用通配符 (wildcards)，并谨慎使用广泛模式；请注意，每条规则既可以匹配项目名称也可以匹配完整路径，并且如果文件夹匹配，则必须忽略其整个分支。删除测试、非必要文档、二进制资产、缓存、日志、构建 (builds)、锁 (locks)、临时工件、机密 (secrets) 和冗余，但保留功能核心、模块、运行时 (runtime)、提供程序、适配器、执行流以及任何支持系统结构理解的内容。\n\n如有疑问，请保留相关代码并仅排除噪音。\n\n优先考虑以下模式：\n*test*\n*tests*\n*docs*\n*example*\n*examples*\n*bench*\n*benchmark*\n*fixture*\n*fixtures*\n*mock*\n*mocks*\n*__pycache__*\n*node_modules*\n*.log\n*.lock\n*.tmp\n*.cache\ndist*\nbuild*\ntarget*\ncoverage*",
