...
window.translations.pt = {
    ...
    step5: {
        ...
        ai: {
            ...
            prompt: { label: "Exemplo de pedido", body: "Você está gerando uma skip list para o Passo 5 do CodeFlatten PRO.\nAnalise o repositório e gere apenas uma skip list otimizada em padrão fnmatch, pronta para uso direto.\n\nObjetivo:\nProduzir a menor skip list possível com alta cobertura, excluindo ruído sem prejudicar o entendimento estrutural do sistema.\n\nPolítica de otimização:\n- Dê preferência a regras compactas e de alto rendimento que removem ramos ruidosos inteiros de forma segura.\n- Favoreça padrões específicos do repositório em vez de exclusões genéricas.\n- Use nomes exatos para ruído isolado.\n- Use wildcards para famílias recorrentes de arquivos.\n- Use padrões amplos de caminho/nome apenas quando visarem claramente conteúdos não nucleares.\n- Como as regras casam com nomes de itens e caminhos completos, e o casamento de uma pasta remove toda a sua subárvore, prefira a supressão no nível da pasta quando todo o ramo for claramente desnecessário.\n- Quando duas regras forem redundantes, mantenha a mais compacta se ela preservar o mesmo poder de exclusão prático.\n- No entanto, não comprima em excesso se isso aumentar o risco de excluir código relevante.\n\nO que remover:\n- testes, probes, smoke checks, helpers de validação, artefatos temporários\n- docs e materiais de handoff que não sejam necessários para a arquitetura\n- assets binários, ícones, outputs de empacotamento, objetos compilados, caches, logs, locks\n- pastas build/dist/release e artefatos gerados\n- segredos, certificados, evidências de runtime, sobras de submissão de store, payloads de localização redundantes quando pesados e desnecessários para entender o fluxo de execução\n\nO que preservar:\n- núcleo funcional\n- módulos, runtime, providers, adapters\n- orquestração, entrypoints, fluxo de execução\n- código que explica como o sistema funciona\n- código de integração de plataforma ou empacotamento quando fizer parte do comportamento do produto ou arquitetura\n\nRegra de decisão:\nNa dúvida, preserve o código relevante e exclua apenas ruídos óbvios.\n\nRegra especial:\nSe a árvore marcar um item ou família com tags como “[Skipped by Step 5]”, “[Skipped]”, “[Binary/Unsupported]” ou equivalente, inclua uma regra de skip correspondente na lista final sempre que for seguro.\n\nEstratégia de saída:\n- Mescle ambos os estilos: insira primeiro regras compactas de ramificação e, em seguida, uma lista mínima padrão de artefatos recorrentes.\n- Dê preferência a um resultado que pareça uma combinação prática de uma skip list orientada ao repositório e uma lista limpa genérica.\n- Desduplique agressivamente.\n- Retorne apenas os padrões fnmatch finais, um por linha, sem comentários.", note: "Use a resposta como ponto de partida. O resultado ideal é revisável, enxuto e alinhado ao objetivo específico do dump — nunca como substituto da revisão humana ou do Content Redactor." }
        },
        ack: {
